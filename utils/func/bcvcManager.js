...
class BCVCManager {
  ...
  isFormingCandle(candleTimestamp, resolution) {
    ...
    if (isForming) {
      console.log(
        `🕐 Candle at ${candleTime.format("YYYY-MM-DD HH:mm")} is still forming (closes at ${closeTime.format("HH:mm:ss")})`,
      );
    }
  ...
  calculateAverageRange(
    candles,
    lookbackPeriod = this.config.bigCandleLookbackPeriod,
  ) {
  ...
  analyzeBCVC(candles) {
    const minRequiredCandles =
      Math.max(this.config.volumePeriod, this.config.bigCandleLookbackPeriod) +
      1;

    if (candles.length < minRequiredCandles) {
      console.log(
        `Not enough data. Need at least ${minRequiredCandles} candles, have ${candles.length}`,
      );
      return null;
    }
    ...
    if (isUpBar && isHighVolume && isBigCandle) {
      ...
      // Maroon: high volume down bar but NOT a big candle (early warning)
      isBCVC = true;
      candleColor = "maroon";
      isBearish = true;
  ...
  async getBCVC(symbol, resolution = "60", maxRetries = 10, retryDelay = 2000) {
    ...
    if (!tfConfig) {
      console.error(
        `❌ Invalid resolution: ${resolution}. Available: ${Object.keys(this.timeframes).join(", ")}`,
      );
      return null;
    }
    ...
    if (needsInit) {
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          if (!response || !response.candles || response.candles.length === 0) {
            console.log(
              `No data for ${symbol} [${resolution}], retry ${retryCount + 1}/${maxRetries}`,
            );
            retryCount++;
          ...
          if (bcvcResult) {
            ...
            console.log(
              `   Volume: ${bcvcResult.volume.toFixed(0)} (${bcvcResult.volumeRatio}x avg) ${bcvcResult.volumeQualified}`,
            );
            console.log(
              `   Range: ${bcvcResult.candleRange.toFixed(2)} (${bcvcResult.rangeRatio}x avg) ${bcvcResult.rangeQualified}`,
            );
            ...
            console.log(
              `❌ BCVC analysis failed for ${symbol} [${resolution}], retry ${retryCount + 1}/${maxRetries}`,
            );
          }
          ...
          console.error(
            `Error initializing BCVC for ${symbol} [${resolution}] (${retryCount + 1}/${maxRetries}):`,
            error.message,
          );
          retryCount++;
          if (retryCount < maxRetries) {
      ...
      console.error(
        `❌ Failed to initialize BCVC for ${symbol} [${resolution}] after ${maxRetries} attempts`,
      );
      return null;
    }
    ...
    while (retryCount < maxRetries) {
      try {
        const validTo = moment();
        const candlesToFetch =
          resolution === "D" ? 10 : resolution === "60" ? 5 : 10;

        let validFrom;
        if (resolution === "D") {
          validFrom = moment().subtract(candlesToFetch, "days");
        } else {
          validFrom = moment().subtract(
            tfConfig.duration * candlesToFetch,
            "minutes",
          );
        }
        ...
        if (!response || !response.candles || response.candles.length === 0) {
          console.log(
            `No candle data for ${symbol} [${resolution}], retry ${retryCount + 1}/${maxRetries}`,
          );
          retryCount++;
          if (retryCount < maxRetries) {
            await this.sleep(retryDelay * retryCount);
          ...
          if (cache && cache.lastAnalysis) {
            console.log(
              `⚠️ Returning cached BCVC values for ${symbol} [${resolution}]`,
            );
            return {
              symbol,
              resolution,
        ...
        let completedCandles = response.candles.filter(
          (candle) => !this.isFormingCandle(candle[0], resolution),
        );

        if (completedCandles.length === 0) {
          console.log(
            `⚠️ No completed candles available yet for ${symbol} [${resolution}]`,
          );
          retryCount++;
          if (retryCount < maxRetries) {
            await this.sleep(retryDelay * retryCount);
        ...
        if (
          fullResponse &&
          fullResponse.candles &&
          fullResponse.candles.length > 0
        ) {
          let fullCandles = fullResponse.candles.filter(
            (candle) => !this.isFormingCandle(candle[0], resolution),
          );
          ...
          if (cache.lastCandleTimestamp === candleTimestamp) {
            console.log(
              `⚠ Skipping update for ${symbol} [${resolution}] - same candle already processed`,
            );
            return {
              symbol,
              resolution,
        ...
        console.error(
          `Error updating BCVC for ${symbol} [${resolution}] (${retryCount + 1}/${maxRetries}):`,
          error.message,
        );
        retryCount++;
        if (retryCount < maxRetries) {
    ...
    if (cache && cache.lastAnalysis) {
      console.log(
        `⚠️ All retries failed, returning cached BCVC values for ${symbol} [${resolution}]`,
      );
      return {
        symbol,
        resolution,
    ...
    console.error(
      `❌ Failed to get BCVC for ${symbol} [${resolution}] after ${maxRetries} attempts`,
    );
    return null;
  }

  async getMultipleBCVC(
    symbols,
    resolution = "60",
    maxRetries = 10,
    retryDelay = 2000,
    ...
    for (const symbol of symbols) {
      results[symbol] = await this.getBCVC(
        symbol,
        resolution,
        maxRetries,
        retryDelay,
  ...
  async getMultiTimeframeBCVC(
    ...
    for (const resolution of resolutions) {
      results[resolution] = await this.getBCVC(
        symbol,
        resolution,
        maxRetries,
        retryDelay,
  ...
  clearSymbolData(symbol, resolution = null) {
    if (resolution) {
      ...
      if (cleared > 0) {
        console.log(
          `✓ Cleared BCVC data for ${symbol} (${cleared} timeframes)`,
        );
      }
    }
  }
  ...
  getCacheStatus() {
    const entries = Array.from(this.bcvcCache.entries()).map(
      ([key, value]) => ({
        key,
        symbol: value.symbol,
  ...
  async getHistoricalBCVC(
    ...
    const shouldIncludeMaroon = includeRed === "maroon"; // NEW

    const originalIncludeRed = this.config.includeRed;
    const originalIncludeMaroon = this.config.includeMaroon; // NEW

    if (shouldIncludeRed) {
      this.config.includeRed = true;
    }
    if (shouldIncludeMaroon) {
      // NEW
      this.config.includeMaroon = true; // NEW
    } // NEW
    ...
    console.log(
      `🔍 Fetching ${days} days of BCVC history for ${symbol} [${resolution}]...`,
    );
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        const redCount = bcvcFormations.filter(
          (f) => f.candleColor === "red",
        ).length;
        const orangeCount = bcvcFormations.filter(
          (f) => f.candleColor === "orange",
        ).length;
        const whiteCount = bcvcFormations.filter(
        ...
        const avgVolumeRatio =
          totalCount > 0
            ? bcvcFormations.reduce((sum, f) => sum + f.volumeRatio, 0) /
              totalCount
            : 0;
        const avgRangeRatio =
          totalCount > 0
            ? bcvcFormations.reduce((sum, f) => sum + f.rangeRatio, 0) /
              totalCount
            : 0;
        ...
        const result = {
          ...
          mostRecent:
            bcvcFormations.length > 0
              ? bcvcFormations[bcvcFormations.length - 1]
              : null,
        };

        this.config.includeRed = originalIncludeRed;
          this.config.includeMaroon = originalIncludeMaroon; 

        return result;
      } catch (error) {
        console.error(
          `Error fetching historical BCVC for ${symbol} [${resolution}] (${retryCount + 1}/${maxRetries}):`,
          error.message,
        );
        retryCount++;
        if (retryCount < maxRetries) {
  ...
  this.config.includeMaroon = originalIncludeMaroon; 
    console.error(
      `❌ Failed to get historical BCVC for ${symbol} [${resolution}] after ${maxRetries} attempts`,
    );
    return null;
  }

  async getMultipleHistoricalBCVC(
    symbols,
    resolution = "60",
    days = 20,
    includeRed = null,
    ...
    for (const symbol of symbols) {
      const historical = await this.getHistoricalBCVC(
        symbol,
        resolution,
        days,
        includeRed,
    ...
    console.log(
      `   🚀 Total Bullish: ${summary.totalBullish} (White: ${summary.totalWhite})`,
    );
    console.log(
      `   🔴 Total Bearish: ${summary.totalBearish} (Orange: ${summary.totalOrange}, Red: ${summary.totalRed})`,
    );
  ...
  async generateBCVCReport(symbol, resolution = "60", days = 20) {
    ...
    const report = {
      ...
      summary: {
        ...
        bullishPercentage:
          historical.bcvcCount > 0
            ? ((historical.bullishCount / historical.bcvcCount) * 100).toFixed(
                2,
              )
            : "0.00",
        bearishPercentage:
          historical.bcvcCount > 0
            ? ((historical.bearishCount / historical.bcvcCount) * 100).toFixed(
                2,
              )
            : "0.00",
      },
    ...
    console.log(
      `BCVC Formations: ${report.summary.bcvcFormations} (${report.summary.bcvcPercentage}% of candles)`,
    );
    console.log(
      `  🚀 Bullish: ${report.summary.bullishFormations} (${report.summary.bullishPercentage}%) - White: ${report.summary.whiteFormations}`,
    );
    console.log(
      `  🔴 Bearish: ${report.summary.bearishFormations} (${report.summary.bearishPercentage}%) - Orange: ${report.summary.orangeFormations}, Red: ${report.summary.redFormations}`,
    );
    ...
    console.log(`${"-".repeat(80)}`);
    console.log(
      `Average Volume Ratio: ${report.characteristics.averageVolumeRatio}x`,
    );
    console.log(
      `Average Range Ratio: ${report.characteristics.averageRangeRatio}x`,
    );
    console.log(`\nConfiguration:`);
    console.log(
      `  Volume Period: ${report.characteristics.configuration.volumePeriod}`,
    );
    console.log(
      `  Volume Threshold: ${report.characteristics.configuration.volumeProportion}x`,
    );
    console.log(
      `  Range Lookback: ${report.characteristics.configuration.bigCandleLookbackPeriod}`,
    );
    console.log(
      `  Range Threshold: ${report.characteristics.configuration.bigCandleProportion}x`,
    );
    console.log(
      `  Include Red Candles: ${report.characteristics.configuration.includeRed}`,
    );
    ...
    if (report.recentActivity.last5Formations.length > 0) {
      ...
      report.recentActivity.last5Formations.forEach((sig, idx) => {
        console.log(
          `${idx + 1}. ${sig.timestamp} | ${sig.type} | Vol: ${sig.volumeRatio.toFixed(2)}x | Range: ${sig.rangeRatio.toFixed(2)}x`,
        );
      });
    }
